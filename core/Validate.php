                    ...
                    switch($rule){
                        case "min":
                            if(strlen($value) < $rule_value){
                                $this->addError(["{$display} must by minimum of {$rule_value} characters.",$item]);
                            }
                        break;
                        case "max":
                            if(strlen($value) > $rule_value){
                                $this->addError(["{$display} must by maximum of {$rule_value} characters.",$item]);
                            }
                        break;
                        case "matches":
                        ...
                        case "unique_update":
                            ...
                            if($query->count()){
                                $this->addError(["{$display} already exists, please coose anoder {$display}.",$item]);   
                            }
                            break;
                        case "is_numeric":
                            if(!is_numeric($value)){
                                $this->addError(["{$display} has to be a number. Please use a numeric value", $ietm]);
                            }    
                            break;
                        case "valid email":
